(() => {
  ...
  function renderPageLaunch() {
    const active = pageLaunch.dataset.active === 'true', state = pageLaunch.dataset.state;
    pageLaunch.textContent = state === 'busy' ? '翻译中' : active ? '原文' : state === 'error' ? '重试' : '翻译';
    pageLaunch.setAttribute('aria-busy', String(state === 'busy'));
  }
  function showPageState(text, state = 'busy') { pageLaunch.dataset.state = state; pageLaunch.title = text; renderPageLaunch(); }
